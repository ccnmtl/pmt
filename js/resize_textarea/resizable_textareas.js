...
Drag = {
   ...
   start: function(e) {
	 ...
	 alert(err);
       ...
       Drag._move.push(connect(document, 'onmousemove', Drag._drag));
       Drag._down.push(connect(document, 'onmouseup', Drag._stop));
   },
	 ...
	 setElementDimensions(ta, {w : Drag._tadim.w + mousediff.x, 
      ...
      _stop: function(e) {
	  forEach(Drag._move,disconnect);
	  forEach(Drag._down,disconnect);
	  Drag._move = [];
	  Drag._down = [];
      }
};
